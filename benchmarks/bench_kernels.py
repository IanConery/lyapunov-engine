...
def benchmark_op(fn, warmup: int = 25, iters: int = 100) -> float:
    ...
    torch.cuda.synchronize()
    times = [s.elapsed_time(e) * 1000.0 for s, e in zip(start_events, end_events)] # microseconds
    times.sort()
...
def run_rmsnorm_benchmarks():
    ...
    print("=" * 70)
    print(f"{'Hidden Dim':<12} | {'Tokens':<10} | {'Eager (us)':<12} | {'Fused (us)':<12} | {'Speedup':<8}")
    print("-" * 70)
    ...
    for hidden_dim in [2048, 4096, 8192]:
        for tokens in [32, 256, 1024, 4096]:
            ...
            def eager_fn():
                ...
                return (x.float() * torch.rsqrt(var + eps) * weight.float()).half()
            ...
            print(f"{hidden_dim:<12} | {tokens:<10} | {eager_us:<12.2f} | {fused_us:<12.2f} | {speedup:<8.2f}x")
...
def run_flash_attn_benchmarks():
    ...
    print("=" * 70)
    print(f"{'Seq Len':<10} | {'Heads':<8} | {'SDPA (us)':<12} | {'FlashAttn (us)':<15} | {'Speedup':<8}")
    print("-" * 70)
    ...
    for seq_len in [512, 1024, 2048, 4096]:
        ...
        k = torch.randn((batch_size, num_kv_heads, seq_len, head_dim), dtype=torch.float16, device=device)
        v = torch.randn((batch_size, num_kv_heads, seq_len, head_dim), dtype=torch.float16, device=device)

        k_rep = k.repeat_interleave(num_heads // num_kv_heads, dim=1)
        v_rep = v.repeat_interleave(num_heads // num_kv_heads, dim=1)

        def sdpa_fn():
            return torch.nn.functional.scaled_dot_product_attention(q, k_rep, v_rep, is_causal=True)

        def flash_fn():
            return flash_attn_v2(q, k, v, is_causal=True)

        sdpa_us = benchmark_op(sdpa_fn)
        flash_us = benchmark_op(flash_fn)
        speedup = sdpa_us / flash_us

        print(f"{seq_len:<10} | {num_heads:<8} | {sdpa_us:<12.2f} | {flash_us:<15.2f} | {speedup:<8.2f}x")
