...
def generate_roofline_data(
    peak_tflops: float = 120.0, peak_bandwidth_gbs: float = 1000.0
):
    ...
    np.minimum(peak_tflops, arithmetic_intensities * (peak_bandwidth_gbs / 1000.0))
    ...
    print("-" * 60)
    print(
        f"Memory-Bound Regime:      Operational Intensity < {ridge_point:.2f} FLOPs/Byte"
    )
    print(
        f"Compute-Bound Regime:     Operational Intensity >= {ridge_point:.2f} FLOPs/Byte"
    )
    print("=" * 60)
