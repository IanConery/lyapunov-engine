...
std::vector<torch::Tensor> rmsnorm_forward(
    ...
    if (input.scalar_type() == at::ScalarType::Float) {
        ...
        TORCH_CHECK(false, "Unsupported data type for rmsnorm: only Float and Half are supported");
    }
...
torch::Tensor swiglu_forward(torch::Tensor gate_up) {
    ...
    if (gate_up.scalar_type() == at::ScalarType::Float) {
        ...
        TORCH_CHECK(false, "Unsupported data type for swiglu: only Float and Half are supported");
    }
...
torch::Tensor flash_attn_v2_fwd_tensor(
    torch::Tensor q,
    torch::Tensor k,
    torch::Tensor v,
    ...
    TORCH_CHECK(q.dim() == 4, "q must have shape [batch, num_heads, seqlen, head_dim]");
    TORCH_CHECK(k.dim() == 4, "k must have shape [batch, num_kv_heads, seqlen, head_dim]");
    TORCH_CHECK(v.dim() == 4, "v must have shape [batch, num_kv_heads, seqlen, head_dim]");
    ...
    float scale = sm_scale.has_value() ? static_cast<float>(*sm_scale) : 1.0f / std::sqrt(static_cast<float>(head_dim));
...
torch::Tensor flash_decoding_tensor(
    torch::Tensor q,
    torch::Tensor k,
    torch::Tensor v,
    int num_partitions,
    ...
    TORCH_CHECK(q.is_cuda() && k.is_cuda() && v.is_cuda(), "All tensors must be on CUDA");
    TORCH_CHECK(q.dim() == 4 && q.size(2) == 1, "q must have shape [batch, num_heads, 1, head_dim]");
    ...
    auto out = torch::empty_like(q);
    auto mid_out = torch::empty({batch_size, num_heads, num_partitions, head_dim}, q.options());
    auto mid_lse = torch::empty({batch_size, num_heads, num_partitions}, q.options().dtype(torch::kFloat32));
...
torch::Tensor paged_attention_v1(
    ...
    TORCH_CHECK(q.is_cuda() && k_cache.is_cuda() && v_cache.is_cuda(), "All tensors must be on CUDA");
    TORCH_CHECK(block_tables.is_cuda() && context_lens.is_cuda(), "Metadata tensors must be on CUDA");
...
torch::Tensor dequantize_q4_0_tensor(torch::Tensor src, int64_t num_elements) {
    ...
    launch_dequantize_q4_0(src.data_ptr(), out.data_ptr<float>(), num_elements, stream);
    return out;
}
...
torch::Tensor dequantize_q8_0_tensor(torch::Tensor src, int64_t num_elements) {
    ...
    auto out = torch::empty({num_elements}, src.options().dtype(torch::kFloat32));
    launch_dequantize_q8_0(src.data_ptr(), out.data_ptr<float>(), num_elements, stream);
    return out;
}
...
torch::Tensor dequantize_q4_k_tensor(torch::Tensor src, int64_t num_elements) {
    ...
    auto out = torch::empty({num_elements}, src.options().dtype(torch::kFloat32));
    launch_dequantize_q4_k(src.data_ptr(), out.data_ptr<float>(), num_elements, stream);
    return out;
}

torch::Tensor quant_gemv_tensor(
    torch::Tensor x,
    ...
    auto y = torch::empty({batch_size, out_features}, x.options().dtype(torch::kFloat32));

    const float* scales_ptr = scales.has_value() ? scales.value().data_ptr<float>() : nullptr;
...
torch::Tensor fp8_gemm_tensor(
    ...
    auto y = torch::empty({m, n}, torch::TensorOptions().dtype(torch::kFloat32).device(x_fp8.device()));
...
torch::Tensor marlin_gemm_tensor(
    torch::Tensor x,
    torch::Tensor qweight,
    torch::Tensor scales
) {
...
void fused_rope_paged_tensor(
    ...
    params.block_tables = block_tables.has_value() ? block_tables.value().data_ptr<int32_t>() : nullptr;
    params.context_lens = context_lens.has_value() ? context_lens.value().data_ptr<int32_t>() : nullptr;
    ...
    params.max_blocks_per_seq = block_tables.has_value() ? block_tables.value().size(1) : 0;
    params.dtype = (q_in.scalar_type() == at::ScalarType::Half) ? CUDA_R_16F : CUDA_R_32F;
...
std::tuple<torch::Tensor, torch::Tensor> speculative_verification_tensor(
    ...
    auto accepted_tokens = torch::zeros({batch_size, num_draft + 1}, torch::TensorOptions().dtype(torch::kInt32).device(target_probs.device()));
    auto num_accepted = torch::zeros({batch_size}, torch::TensorOptions().dtype(torch::kInt32).device(target_probs.device()));

    launch_speculative_verification(
        target_probs.data_ptr<float>(),
        draft_probs.data_ptr<float>(),
        draft_tokens.data_ptr<int32_t>(),
        rand_uniform.data_ptr<float>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("rmsnorm_forward", &rmsnorm_forward, "Fused RMSNorm forward kernel",
          py::arg("input"), py::arg("weight"), py::arg("eps") = 1e-5, py::arg("residual") = py::none());
    m.def("swiglu_forward", &swiglu_forward, "Fused SwiGLU forward kernel",
          py::arg("gate_up"));
    m.def("flash_attn_v2_fwd", &flash_attn_v2_fwd_tensor, "FlashAttention-2 forward kernel",
          py::arg("q"), py::arg("k"), py::arg("v"), py::arg("sm_scale") = py::none(), py::arg("is_causal") = true);
    m.def("flash_decoding", &flash_decoding_tensor, "FlashDecoding split-KV kernel",
          py::arg("q"), py::arg("k"), py::arg("v"), py::arg("num_partitions") = 4, py::arg("sm_scale") = py::none());
    m.def("paged_attention_v1", &paged_attention_v1, "PagedAttention v1 decoding kernel",
          py::arg("q"), py::arg("k_cache"), py::arg("v_cache"), py::arg("block_tables"), py::arg("context_lens"), py::arg("sm_scale") = py::none());
    m.def("fused_rope_paged", &fused_rope_paged_tensor, "Fused RoPE and paged KV write kernel",
          py::arg("q_out"), py::arg("k_cache"), py::arg("v_cache"), py::arg("q_in"), py::arg("k_in"), py::arg("v_in"), py::arg("cos_sin_cache"), py::arg("block_tables") = py::none(), py::arg("context_lens") = py::none());
    m.def("speculative_verification", &speculative_verification_tensor, "Speculative decoding parallel verification",
          py::arg("target_probs"), py::arg("draft_probs"), py::arg("draft_tokens"), py::arg("rand_uniform"));

    // Quantization Kernels
    m.def("dequantize_q4_0", &dequantize_q4_0_tensor, "Dequantize Q4_0 block tensor to FP32",
          py::arg("src"), py::arg("num_elements"));
    m.def("dequantize_q8_0", &dequantize_q8_0_tensor, "Dequantize Q8_0 block tensor to FP32",
          py::arg("src"), py::arg("num_elements"));
    m.def("dequantize_q4_k", &dequantize_q4_k_tensor, "Dequantize Q4_K super-block tensor to FP32",
          py::arg("src"), py::arg("num_elements"));
    m.def("quant_gemv", &quant_gemv_tensor, "Quantized GEMV matrix multiplication",
          py::arg("x"), py::arg("qweight"), py::arg("scales") = py::none(), py::arg("in_features"), py::arg("out_features"), py::arg("qtype_enum"));
    m.def("fp8_gemm", &fp8_gemm_tensor, "FP8 E4M3 matrix multiplication with scaling",
          py::arg("x_fp8"), py::arg("w_fp8"), py::arg("scale_x") = 1.0, py::arg("scale_w") = 1.0);
    m.def("marlin_gemm", &marlin_gemm_tensor, "Marlin W4A16 matrix multiplication",
          py::arg("x"), py::arg("qweight"), py::arg("scales"));
    ...
    py::class_<Sequence, SequencePtr>(m, "Sequence")
        .def(py::init<int64_t, const std::vector<int32_t>&, const SamplingParams&>())
    ...
    py::class_<BlockSpaceManager, std::shared_ptr<BlockSpaceManager>>(m, "BlockSpaceManager")
        .def(py::init<int, int, bool>(), py::arg("num_blocks"), py::arg("block_size") = 16, py::arg("enable_prefix_caching") = true)
    ...
    py::class_<ContinuousScheduler>(m, "ContinuousScheduler")
        .def(py::init<const SchedulerConfig&, std::shared_ptr<BlockSpaceManager>>())
        .def("add_sequence", &ContinuousScheduler::add_sequence)
        .def("abort_sequence", &ContinuousScheduler::abort_sequence)
        .def("has_unfinished_sequences", &ContinuousScheduler::has_unfinished_sequences)
        .def("get_num_waiting_sequences", &ContinuousScheduler::get_num_waiting_sequences)
        .def("get_num_running_sequences", &ContinuousScheduler::get_num_running_sequences)
