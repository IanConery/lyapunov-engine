...
void CUDAGraphRunner::capture(
    ...
    LYAPUNOV_LOG_INFO("Successfully captured CUDA Graph for batch size " << batch_size);
}

void CUDAGraphRunner::replay(int batch_size, cudaStream_t stream) {
    auto it = graph_execs_.find(batch_size);
    if (it == graph_execs_.end()) {
        throw std::runtime_error("CUDA Graph not captured for batch size " + std::to_string(batch_size));
    }
