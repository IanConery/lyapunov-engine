...
void ContinuousScheduler::post_step(const std::vector<SequencePtr> &seqs,
                                    const std::vector<int32_t> &next_tokens) {
  if (seqs.size() != next_tokens.size()) {
    throw std::runtime_error(
        "Size mismatch between scheduled seqs and next tokens");
  }
  ...
  for (size_t i = 0; i < seqs.size(); ++i) {
    ...
    bool is_length_capped =
        (static_cast<int>(seq->get_output_len()) >= sampling.max_tokens);
