...
public:
    BlockSpaceManager(int num_blocks, int block_size = 16, bool enable_prefix_caching = true);

    int get_num_blocks() const { return num_blocks_; }
    int get_block_size() const { return block_size_; }
    int get_num_free_blocks() const { return static_cast<int>(free_blocks_.size()); }
    int get_num_used_blocks() const { return num_blocks_ - get_num_free_blocks(); }
