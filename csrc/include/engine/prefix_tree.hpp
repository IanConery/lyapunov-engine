...
struct PrefixTreeNode {
  ...
  PrefixTreeNode(int block_id = -1)
      : physical_block_id(block_id), ref_count(1) {}
};
...
public:
  ...
  std::vector<int>
  match_prefix(const std::vector<int32_t> &prompt_tokens) const {
  ...
  void insert_prefix(const std::vector<int32_t> &prompt_tokens,
                     const std::vector<int> &physical_blocks) {
    size_t num_blocks =
        std::min(prompt_tokens.size() / block_size_, physical_blocks.size());
    auto curr = root_;
