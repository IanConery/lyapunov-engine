...
public:
    ContinuousScheduler(const SchedulerConfig& config, std::shared_ptr<BlockSpaceManager> block_manager);

    void add_sequence(SequencePtr seq);
    void abort_sequence(int64_t seq_id);

    bool has_unfinished_sequences() const;
    int get_num_waiting_sequences() const { return static_cast<int>(waiting_.size()); }
    int get_num_running_sequences() const { return static_cast<int>(running_.size()); }

    SchedulerOutputs schedule();
    void post_step(const std::vector<SequencePtr>& seqs, const std::vector<int32_t>& next_tokens);
