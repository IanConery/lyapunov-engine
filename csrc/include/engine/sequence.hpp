...
class Sequence {
public:
    Sequence(int64_t seq_id, const std::vector<int32_t>& prompt_tokens, const SamplingParams& sampling_params)
        : seq_id_(seq_id),
          prompt_token_ids_(prompt_tokens),
          output_token_ids_(),
    ...
    const std::vector<int32_t>& get_prompt_tokens() const { return prompt_token_ids_; }
    const std::vector<int32_t>& get_output_tokens() const { return output_token_ids_; }
    ...
    size_t get_total_len() const { return prompt_token_ids_.size() + output_token_ids_.size(); }
    ...
    void set_block_table(const std::vector<int32_t>& table) { block_table_ = table; }
