...
#define CUDA_CHECK(expr)                                                       \
  do {                                                                         \
    ...
    if (err != cudaSuccess) {                                                  \
      std::stringstream ss;                                                    \
      ss << "CUDA error at " << __FILE__ << ":" << __LINE__ << " code=" << err \
...
#define CUDA_CHECK_LAST_ERROR()                                                \
  do {                                                                         \
    ...
    if (err != cudaSuccess) {                                                  \
      std::stringstream ss;                                                    \
      ss << "CUDA kernel launch error at " << __FILE__ << ":" << __LINE__      \
...
template <typename T, int N> struct alignas(sizeof(T) * N) VecType {
  T data[N];
};
...
template <typename T> __inline__ __device__ float to_float(T val) {
  return static_cast<float>(val);
}

template <> __inline__ __device__ float to_float<half>(half val) {
  return __half2float(val);
}

template <typename T> __inline__ __device__ T from_float(float val) {
  return static_cast<T>(val);
}

template <> __inline__ __device__ half from_float<half>(float val) {
