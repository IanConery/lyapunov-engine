...
__global__ void flash_attn_v2_fwd_kernel(
    ...
    float* lse_ptr = lse ? (lse + (batch_idx * num_heads + head_idx) * q_seqlen) : nullptr;

    extern __shared__ char smem_buffer[];
    T* s_q = reinterpret_cast<T*>(smem_buffer);
    T* s_k = s_q + BLOCK_M * HEAD_DIM;
    T* s_v = s_k + BLOCK_N * HEAD_DIM;
    float* s_p = reinterpret_cast<float*>(s_v + BLOCK_N * HEAD_DIM); // [BLOCK_M, BLOCK_N]
    ...
    const int elems_per_thread = (BLOCK_M * HEAD_DIM + num_threads - 1) / num_threads;
    float thread_acc[32]; // Max 32 elements per thread for 32x128 / 128 threads = 32
            ...
            if (global_m < q_seqlen && global_n < kv_seqlen) {
                if (is_causal && global_n > global_m) {
                    ...
                    for (int d = 0; d < HEAD_DIM; ++d) {
                        score += cuda::to_float(s_q[m * HEAD_DIM + d]) * cuda::to_float(s_k[n * HEAD_DIM + d]);
                    }
    ...
    for (int i = 0; i < elems_per_thread; ++i) {
        ...
        if (idx < BLOCK_M * HEAD_DIM) {
            ...
            if (global_m < q_seqlen) {
                ...
                out_ptr[global_m * out_seq_stride + d] = cuda::from_float<T>(thread_acc[i] * inv_sum);
            }
        }
    }
    ...
    if (lse_ptr && tid < BLOCK_M) {
        ...
        if (global_m < q_seqlen) {
            lse_ptr[global_m] = (s_row_sum[tid] > 0.0f) ? (s_row_max[tid] + logf(s_row_sum[tid])) : -1e20f;
        }
    }
}
...
void launch_flash_attn_v2(const FlashAttnParams& params, cudaStream_t stream) {
    ...
    size_t element_size = (params.dtype == CUDA_R_32F) ? sizeof(float) : sizeof(half);
    size_t smem_bytes = (BLOCK_M * params.head_dim + 2 * BLOCK_N * params.head_dim) * element_size
    ...
    if (params.dtype == CUDA_R_32F) {
        switch (params.head_dim) {
            case 64:
                flash_attn_v2_fwd_kernel<float, 64><<<grid, block, smem_bytes, stream>>>(
                    ...
                    params.q_seqlen, params.kv_seqlen,
                    params.q_batch_stride, params.q_head_stride, params.q_seq_stride,
                    ...
                    params.out_batch_stride, params.out_head_stride, params.out_seq_stride,
                    params.sm_scale, params.is_causal
                );
                break;
            case 128:
                if (smem_bytes > 48 * 1024) {
                    cudaFuncSetAttribute(
                        flash_attn_v2_fwd_kernel<float, 128>,
                ...
                break;
            default:
                throw std::invalid_argument("Unsupported head dimension for FP32 flash_attn_v2");
        }
        ...
        switch (params.head_dim) {
            ...
            case 128:
                ...
                break;
            default:
                throw std::invalid_argument("Unsupported head dimension for FP16 flash_attn_v2");
        }
    } else {
