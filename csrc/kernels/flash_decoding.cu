...
__global__ void flash_decoding_stage1_kernel(
    ...
    int mid_idx = (batch_idx * num_heads + head_idx) * num_partitions + partition_idx;
    ...
    if (tid < HEAD_DIM) {
        for (int p = 0; p < num_partitions; ++p) {
            ...
            if (lse_p > -1e10f) {
                ...
                float val = cuda::to_float(mid_out[(base_mid_idx + p) * HEAD_DIM + tid]);
...
void launch_flash_decoding(const FlashDecodingParams& params, cudaStream_t stream) {
    int partition_size = (params.kv_seqlen + params.num_partitions - 1) / params.num_partitions;
        ...
        switch (params.head_dim) {
            case 64:
                flash_decoding_stage1_kernel<float, 64><<<grid1, block1, 0, stream>>>(
                    ...
                    static_cast<float*>(params.mid_out_ptr),
                    params.mid_lse_ptr,
                    params.num_heads, params.num_kv_heads,
                    params.kv_seqlen, params.num_partitions, partition_size,
                    params.q_batch_stride, params.q_head_stride,
                    params.k_batch_stride, params.k_head_stride, params.k_seq_stride,
                    params.v_batch_stride, params.v_head_stride, params.v_seq_stride,
                    params.sm_scale
                );
                flash_decoding_stage2_reduce_kernel<float, 64><<<grid2, block2, 0, stream>>>(
                    static_cast<const float*>(params.mid_out_ptr),
                    params.mid_lse_ptr,
                    static_cast<float*>(params.out_ptr),
                    params.num_heads, params.num_partitions,
                    params.out_batch_stride, params.out_head_stride
                );
                break;
            case 128:
                ...
                flash_decoding_stage2_reduce_kernel<float, 128><<<grid2, block2, 0, stream>>>(
                    static_cast<const float*>(params.mid_out_ptr),
                    params.mid_lse_ptr,
                    static_cast<float*>(params.out_ptr),
                    params.num_heads, params.num_partitions,
                ...
                break;
            default:
                throw std::invalid_argument("Unsupported head dimension for FP32 flash_decoding");
        }
        ...
        switch (params.head_dim) {
            case 64:
                flash_decoding_stage1_kernel<half, 64><<<grid1, block1, 0, stream>>>(
                    ...
                    static_cast<half*>(params.mid_out_ptr),
                    params.mid_lse_ptr,
                    params.num_heads, params.num_kv_heads,
                    params.kv_seqlen, params.num_partitions, partition_size,
                    params.q_batch_stride, params.q_head_stride,
                ...
                flash_decoding_stage2_reduce_kernel<half, 64><<<grid2, block2, 0, stream>>>(
                    static_cast<const half*>(params.mid_out_ptr),
                    params.mid_lse_ptr,
            ...
            case 128:
                ...
                flash_decoding_stage2_reduce_kernel<half, 128><<<grid2, block2, 0, stream>>>(
                    static_cast<const half*>(params.mid_out_ptr),
                    params.mid_lse_ptr,
                    static_cast<half*>(params.out_ptr),
                    params.num_heads, params.num_partitions,
                ...
                break;
            default:
                throw std::invalid_argument("Unsupported head dimension for FP16 flash_decoding");
        }
    } else {
