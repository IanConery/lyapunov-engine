...
void launch_rmsnorm(
    ...
    if (hidden_dim <= 512) threads_per_block = 128;
    else if (hidden_dim >= 4096) threads_per_block = 512;
    ...
    if (dtype == CUDA_R_32F) {
        rmsnorm_kernel<float><<<grid, block, 0, stream>>>(
            ...
            static_cast<float*>(out_residual),
...
void launch_swiglu(
    ...
    if (dtype == CUDA_R_32F) {
        swiglu_kernel<float, VEC_SIZE><<<blocks, threads_per_block, 0, stream>>>(
            static_cast<float*>(out),
            static_cast<const float*>(gate_up),
            intermediate_dim,
            total_elements
        );
    } else if (dtype == CUDA_R_16F) {
