...
__global__ void fused_rope_paged_kernel(
    ...
    const T *__restrict__ q_in, const T *__restrict__ k_in,
    const T *__restrict__ v_in, const float *__restrict__ cos_sin_cache,
    const int32_t *__restrict__ block_tables,
    const int32_t *__restrict__ context_lens,
    const int32_t *__restrict__ seq_idx_map, int num_tokens, int num_heads,
  ...
  if (head_idx < num_kv_heads && block_tables != nullptr) {
    ...
    int physical_block_id =
        block_tables[seq_idx * max_blocks_per_seq + block_num];

    const T *k_src = k_in + (token_idx * num_kv_heads + head_idx) * head_dim;
    const T *v_src = v_in + (token_idx * num_kv_heads + head_idx) * head_dim;

    // Strides in physical KV cache: [num_blocks, num_kv_heads, block_size,
    // head_dim]
    int kv_cache_offset =
        ((physical_block_id * num_kv_heads + head_idx) * block_size +
         block_offset) *
        head_dim;
...
void launch_fused_rope_paged(const FusedRoPEParams &params,
  ...
  if (params.dtype == CUDA_R_16F) {
    fused_rope_paged_kernel<half><<<grid, block, 0, stream>>>(
        static_cast<half *>(params.q_out), static_cast<half *>(params.k_cache),
        static_cast<half *>(params.v_cache),
        static_cast<const half *>(params.q_in),
        static_cast<const half *>(params.k_in),
        static_cast<const half *>(params.v_in), params.cos_sin_cache,
        params.block_tables, params.context_lens, params.seq_idx_map,
        params.num_tokens, params.num_heads, params.num_kv_heads,
        params.head_dim, params.block_size, params.max_blocks_per_seq);
  } else {
    fused_rope_paged_kernel<float><<<grid, block, 0, stream>>>(
        ...
        static_cast<const float *>(params.v_in), params.cos_sin_cache,
        params.block_tables, params.context_lens, params.seq_idx_map,
        params.num_tokens, params.num_heads, params.num_kv_heads,
        params.head_dim, params.block_size, params.max_blocks_per_seq);
  }
