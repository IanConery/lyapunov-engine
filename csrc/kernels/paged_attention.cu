...
__global__ void paged_attention_kernel(
    ...
    const int32_t* block_table = block_tables + batch_idx * max_num_blocks_per_seq;
    ...
    for (int block_idx = 0; block_idx < num_blocks; ++block_idx) {
        ...
        const T* k_block_ptr = k_cache + physical_block_idx * k_block_stride + kv_head_idx * k_head_stride;
        const T* v_block_ptr = v_cache + physical_block_idx * v_block_stride + kv_head_idx * v_head_stride;
...
void launch_paged_attention(const PagedAttentionParams& params, cudaStream_t stream) {
        ...
        switch (params.head_dim) {
            case 64:
                paged_attention_kernel<float, 64><<<grid, block, 0, stream>>>(
                    ...
                    static_cast<const float*>(params.v_cache_ptr),
                    params.block_tables,
                    params.context_lens,
                    params.num_heads,
                    params.num_kv_heads,
                    ...
                    params.k_block_stride, params.k_head_stride, params.k_token_stride,
                    params.v_block_stride, params.v_head_stride, params.v_token_stride,
            ...
            case 128:
                ...
                break;
            default:
                throw std::invalid_argument("Unsupported head dimension for FP32 paged_attention");
        }
        ...
        switch (params.head_dim) {
            case 64:
                paged_attention_kernel<half, 64><<<grid, block, 0, stream>>>(
                    ...
                    static_cast<const half*>(params.v_cache_ptr),
                    params.block_tables,
                    params.context_lens,
                    params.num_heads,
                    params.num_kv_heads,
            ...
            case 128:
                ...
                break;
            default:
                throw std::invalid_argument("Unsupported head dimension for FP16 paged_attention");
        }
    } else {
