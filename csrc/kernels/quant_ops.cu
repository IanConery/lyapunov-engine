...
__global__ void dequantize_q4_0_kernel(const BlockQ4_0 *__restrict__ src,
...
__global__ void dequantize_q8_0_kernel(const BlockQ8_0 *__restrict__ src,
                                       float *__restrict__ dst,
                                       int num_blocks) {
  int block_idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (block_idx >= num_blocks)
...
__global__ void dequantize_q4_k_kernel(const BlockQ4_K *__restrict__ src,
                                       float *__restrict__ dst,
                                       int num_super_blocks) {
  int sb_idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (sb_idx >= num_super_blocks)
    return;
...
__global__ void quant_gemv_q4_0_kernel(const float *__restrict__ x,
...
#pragma unroll
  ...
  float total_sum =
      cuda::block_reduce_sum<float, BLOCK_THREADS>(sum, s_scratch);
...
template <int BLOCK_THREADS = 128>
__global__ void quant_gemv_q8_0_kernel(const float *__restrict__ x,
...
template <int BLOCK_THREADS = 128>
__global__ void
fp8_gemm_kernel(const uint8_t *__restrict__ x_fp8,
                const uint8_t *__restrict__ w_fp8, float scale_x, float scale_w,
                float *__restrict__ y, int in_features, int out_features) {
  int row = blockIdx.x;
  int batch_idx = blockIdx.y;
  int tid = threadIdx.x;
  ...
  for (int k = tid; k < in_features; k += BLOCK_THREADS) {
    ...
    int exp_x = (bx >> 3) & 0x0F;
    int frac_x = bx & 0x07;
    fx *= (exp_x == 0) ? (frac_x / 8.0f) * 0.125f
                       : (1.0f + frac_x / 8.0f) * powf(2.0f, exp_x - 7);

    float fw = (bw & 0x80) ? -1.0f : 1.0f;
    int exp_w = (bw >> 3) & 0x0F;
    int frac_w = bw & 0x07;
    fw *= (exp_w == 0) ? (frac_w / 8.0f) * 0.125f
                       : (1.0f + frac_w / 8.0f) * powf(2.0f, exp_w - 7);
...
// ----------------------------------------------------------------------------
template <int BLOCK_THREADS = 128>
__global__ void marlin_w4a16_gemv_kernel(const float *__restrict__ x,
...
void launch_dequantize_q4_0(const void *src, float *dst, int num_elements,
                            cudaStream_t stream) {
  int num_blocks = num_elements / 32;
  int threads = 128;
  int grid = (num_blocks + threads - 1) / threads;
  dequantize_q4_0_kernel<<<grid, threads, 0, stream>>>(
      static_cast<const BlockQ4_0 *>(src), dst, num_blocks);
  CUDA_CHECK_LAST_ERROR();
}

void launch_dequantize_q8_0(const void *src, float *dst, int num_elements,
                            cudaStream_t stream) {
  int num_blocks = num_elements / 32;
  int threads = 128;
  int grid = (num_blocks + threads - 1) / threads;
  dequantize_q8_0_kernel<<<grid, threads, 0, stream>>>(
      static_cast<const BlockQ8_0 *>(src), dst, num_blocks);
  CUDA_CHECK_LAST_ERROR();
}

void launch_dequantize_q4_k(const void *src, float *dst, int num_elements,
                            cudaStream_t stream) {
  int num_sb = num_elements / 256;
  int threads = 128;
  int grid = (num_sb + threads - 1) / threads;
  dequantize_q4_k_kernel<<<grid, threads, 0, stream>>>(
      static_cast<const BlockQ4_K *>(src), dst, num_sb);
  CUDA_CHECK_LAST_ERROR();
}

void launch_quant_gemv(const float *x, const void *qweight, const float *scales,
                       float *y, int batch_size, int in_features,
                       int out_features, QuantType qtype, cudaStream_t stream) {
  dim3 grid(out_features, batch_size);
  dim3 block(128);

  switch (qtype) {
  case QuantType::Q4_0:
    quant_gemv_q4_0_kernel<128>
        <<<grid, block, 0, stream>>>(x, static_cast<const BlockQ4_0 *>(qweight),
                                     y, in_features, out_features);
    break;
  case QuantType::Q8_0:
    quant_gemv_q8_0_kernel<128>
        <<<grid, block, 0, stream>>>(x, static_cast<const BlockQ8_0 *>(qweight),
                                     y, in_features, out_features);
    break;
  case QuantType::MARLIN_W4A16:
    marlin_w4a16_gemv_kernel<128>
        <<<grid, block, 0, stream>>>(x, static_cast<const int32_t *>(qweight),
                                     scales, y, in_features, out_features);
    break;
  default:
    throw std::invalid_argument(
        "Unsupported quantization format in launch_quant_gemv");
  }
  CUDA_CHECK_LAST_ERROR();
}

void launch_fp8_gemm(const void *x_fp8, const void *w_fp8, const float *scale_x,
                     const float *scale_w, float *y, int m, int k, int n,
                     cudaStream_t stream) {
  dim3 grid(n, m);
  dim3 block(128);
  ...
  fp8_gemm_kernel<128><<<grid, block, 0, stream>>>(
      static_cast<const uint8_t *>(x_fp8), static_cast<const uint8_t *>(w_fp8),
      sx, sw, y, k, n);
  CUDA_CHECK_LAST_ERROR();
}

void launch_marlin_gemm(const float *x, const int32_t *qweight,
                        const float *scales, float *y, int m, int k, int n,
                        cudaStream_t stream) {
  dim3 grid(n, m);
  dim3 block(128);

  marlin_w4a16_gemv_kernel<128>
      <<<grid, block, 0, stream>>>(x, qweight, scales, y, k, n);
  CUDA_CHECK_LAST_ERROR();
}
