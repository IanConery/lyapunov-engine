...
def load_llama_config_from_hf(model_dir_or_repo: str) -> LlamaConfig:
    ...
    return LlamaConfig(
        ...
        head_dim=cfg_dict.get(
            "head_dim",
            cfg_dict.get("hidden_size", 2048)
            // cfg_dict.get("num_attention_heads", 32),
        ),
...
def load_safetensors_weights(
    model_dir_or_repo: str, device: str = "cpu"
    ...
    if os.path.isdir(model_dir_or_repo):
        ...
        if not files:
            raise FileNotFoundError(
                f"No .safetensors files found in {model_dir_or_repo}"
            )
        ...
        download_dir = snapshot_download(
            repo_id=model_dir_or_repo, allow_patterns=["*.safetensors", "*.json"]
        )
...
def load_hf_weights_into_model(
    ...
    if "model.embed_tokens.weight" in weights:
        model.embed_tokens.weight.data.copy_(
            weights["model.embed_tokens.weight"].to(dtype=dtype, device=device)
        )

    if "model.norm.weight" in weights:
        model.norm_weight.data.copy_(
            weights["model.norm.weight"].to(dtype=dtype, device=device)
        )

    # 2. LM Head (handle tied weights if lm_head is omitted)
    if "lm_head.weight" in weights:
        model.lm_head.weight.data.copy_(
            weights["lm_head.weight"].to(dtype=dtype, device=device)
        )
    ...
    for i in range(num_layers):
        ...
        if in_norm_key in weights:
            layer.input_layernorm_weight.data.copy_(
                weights[in_norm_key].to(dtype=dtype, device=device)
            )
        ...
        if post_norm_key in weights:
            layer.post_attention_layernorm_weight.data.copy_(
                weights[post_norm_key].to(dtype=dtype, device=device)
            )
        ...
        if down_key in weights:
            layer.mlp.down_proj.weight.data.copy_(
                weights[down_key].to(dtype=dtype, device=device)
            )
...
def from_pretrained_llama(
    model_dir_or_repo: str, dtype: torch.dtype = torch.float16, device: str = "cpu"
) -> LlamaForCausalLM:
