...
def rmsnorm(
    ...
    if HAS_CUDA_EXT and x.is_cuda and weight.is_cuda:
        ...
        res_2d = (
            residual.contiguous().view(-1, orig_shape[-1])
...
def flash_attn_v2(
    ...
    if HAS_CUDA_EXT and q.is_cuda:
        return _C.flash_attn_v2_fwd(
            q.contiguous(), k.contiguous(), v.contiguous(), sm_scale, is_causal
        )
    ...
    if is_causal:
        ...
        mask = torch.triu(
            torch.ones(q_len, k_len, device=q.device, dtype=torch.bool), diagonal=1
        )
...
def flash_decoding(
    ...
    if HAS_CUDA_EXT and q.is_cuda:
        return _C.flash_decoding(
            q.contiguous(), k.contiguous(), v.contiguous(), num_partitions, sm_scale
        )
...
def paged_attention(
    ...
    for b in range(batch_size):
        ...
        k_seq = torch.cat(k_slices, dim=1)[
            :, :c_len, :
        ]  # [num_kv_heads, c_len, head_dim]
