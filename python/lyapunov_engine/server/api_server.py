...
def run_stability_pipeline(
    engine: LLMEngine, tokenizer, prompt_tokens: list[int], max_tokens: int = 64
) -> StabilityDiagnostics:
    """Run dynamic safety-gated stability diagnostics (Lyapunov + Semantic Entropy)."""
    context_len = len(prompt_tokens)
    free_gb = 16.0
    if torch.cuda.is_available():
        free_bytes, _total_bytes = torch.cuda.mem_get_info(engine.device)
...
def create_app(
    engine: LLMEngine | None = None, tokenizer=None, model_name: str = "lyapunov-model"
) -> FastAPI:
    ...
    async def chat_completions(req: ChatCompletionRequest):
                    ...
                    for out in outputs:
                        if out.request_id == request_id:
                            ...
                            if len(curr_tokens) > prev_len:
                                ...
                                text_chunk = app.state.tokenizer.decode(
        ...
        completion_text = app.state.tokenizer.decode(
            final_output.output_tokens, skip_special_tokens=True
        )
