...
def test_flash_attn_v2_numerical_parity(batch_size, num_heads, num_kv_heads, q_len, kv_len, head_dim, dtype):
    ...
    q = torch.randn((batch_size, num_heads, q_len, head_dim), dtype=dtype, device=device)
...
@pytest.mark.parametrize("head_dim", [64, 128])
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_flash_decoding_parity(batch_size, num_heads, num_kv_heads, kv_len, head_dim, dtype):
    if not torch.cuda.is_available():
        pytest.skip("CUDA not available")
