...
def precompute_cos_sin(
    max_pos: int, head_dim: int, device: torch.device
) -> torch.Tensor:
    inv_freq = 1.0 / (
...
@pytest.mark.parametrize(
    ...
    [(1, 8, 2, 64), (4, 16, 4, 128), (16, 32, 8, 128)],
)
def test_fused_rope_paged(num_tokens, num_heads, num_kv_heads, head_dim):
    ...
    k_in = torch.randn(
        (num_tokens, num_kv_heads, head_dim), dtype=torch.float32, device=device
    )
    v_in = torch.randn(
        (num_tokens, num_kv_heads, head_dim), dtype=torch.float32, device=device
    )
    ...
    block_tables = torch.zeros(
        (num_tokens, max_blocks), dtype=torch.int32, device=device
    )
    ...
    _C.fused_rope_paged(
        q_out, k_cache, v_cache, q_in, k_in, v_in, cos_sin, block_tables, context_lens
    )
