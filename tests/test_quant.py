...
from lyapunov_engine.models.quant import QuantType
...
def create_synthetic_q4_0_bytes(num_blocks: int, device: torch.device) -> torch.Tensor:
    ...
    scales = (
        torch.rand(num_blocks, dtype=torch.float16, device=device) * 0.1 + 0.01
    ).view(torch.uint8)  # 2 bytes each
...
@pytest.mark.parametrize(
    "in_features,out_features", [(64, 128), (256, 512), (1024, 1024)]
)
def test_q4_0_dequantize_and_gemv(in_features, out_features):
    ...
    dequant = _C.dequantize_q4_0(raw_bytes, out_features * in_features).view(
    ...
    out = _C.quant_gemv(
        x, raw_bytes, None, in_features, out_features, int(QuantType.Q4_0)
    )
...
def test_q8_0_dequantize_and_gemv(in_features, out_features):
    ...
    dequant = _C.dequantize_q8_0(raw_bytes, out_features * in_features).view(
        out_features, in_features
    )
    assert dequant.shape == (out_features, in_features)
    assert not torch.isnan(dequant).any()
    ...
    x = torch.randn((2, in_features), dtype=torch.float32, device=device)
    out = _C.quant_gemv(
        x, raw_bytes, None, in_features, out_features, int(QuantType.Q8_0)
    )
    ref_out = torch.matmul(x, dequant.t())
